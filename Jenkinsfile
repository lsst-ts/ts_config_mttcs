...
pipeline {
    ...
    environment {
        // Python module name.
        MODULE_NAME = 'lsst.ts.config.mttcs'
            ...
            steps {
                ...
                withEnv(["WHOME=${env.WORKSPACE}"]) {
                    sh """
                        ...
                        # Update CSC packages
                        update_or_clone_package ts_MTAlignment
                        # MTAOS has some tricky dependencies; Tiago is working on its
                        update_or_clone_package ts_mthexapod
                        update_or_clone_package ts_m2  # for MTM2
                        update_or_clone_package ts_mtmount
                        update_or_clone_package ts_mtrotator
                     """
                }
            }
        }
